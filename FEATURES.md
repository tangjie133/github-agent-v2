...
| **近似最近邻** | O(log n) 复杂度，支持万级文档 | ChromaDB |
| **自动持久化** | ChromaDB 自动落盘，重启不丢失 | ChromaVectorStore |
| **动态扩容** | 索引自动增长，无需预分配 | ChromaDB |

### PDF 处理优化（新）

| 优化 | 效果 | 文件 |
|------|------|------|
| **PDF 直接处理** | 不按页转 Markdown，直接解析存储 | pdf_processor.py |
| **多线程并行** | 8 线程同时处理多页（可配置） | PDFProcessor |
| **后台异步加载** | 启动时不阻塞，后台处理 PDF | kb_service.py |
| **进度日志** | 实时显示处理进度和性能统计 | pdf_processor.py |
| **智能元数据** | 自动提取 vendor/chip/page | PDFMetadata |
...
| **PDF 处理速度** | **~2页/s** | **~8页/s** | **4x** 🚀 |
| **服务启动时间** | **~30s** | **~3s** | **10x** 🚀 |

### PDF 处理优化

**问题：** 大 PDF 文件（100+页）处理缓慢，阻塞服务启动

**解决方案：** 
1. **直接处理**：不再按页转 Markdown，直接解析 PDF 生成向量
2. **多线程并行**：8 线程同时处理不同页面
3. **后台异步**：启动后立即完成，PDF 在后台处理
4. **增量更新**：只处理新增/变更的 PDF

| 处理方式 | 110 页 PDF 耗时 | 启动阻塞 | 精度 |
|---------|----------------|---------|------|
| **PDF→Markdown→向量** | ~60s | ✅ 阻塞 | 丢失分页 |
| **PDF 直接处理（单线程）** | ~55s | ✅ 阻塞 | 页级精度 |
| **PDF 直接处理（多线程）** | **~15s** | ❌ 后台 | 页级精度 |

**配置参数：**
```bash
KB_PDF_WORKERS=8                    # 并行线程数（默认 CPU/3）
KB_PDF_PARALLEL_THRESHOLD=3         # 启用并行阈值（页）
```
